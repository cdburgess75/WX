...
4. **radar**, the only source that looks at your actual roof: one RainViewer tile at z8 (~500 m/px), the ±1.6 km box of pixels over your coordinates read off a canvas, ≥20% coverage = rain, red-dominant pixels = heavy. Needs CORS on the tile host; `crossOrigin="anonymous"` means it fails closed to `null` if that is ever withdrawn, and the other three carry on.
